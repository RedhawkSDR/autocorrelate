...
if [ "$1" = "rpm" ]; then
    ...
    if [ -e autocorrelate.spec ]; then
        mydir=`dirname $0`
        tmpdir=`mktemp -d`
        cp -r ${mydir} ${tmpdir}/autocorrelate-1.0.1
        tar czf ${tmpdir}/autocorrelate-1.0.1.tar.gz --exclude=".svn" -C ${tmpdir} autocorrelate-1.0.1
        rpmbuild -ta ${tmpdir}/autocorrelate-1.0.1.tar.gz
        rm -rf $tmpdir
    else
